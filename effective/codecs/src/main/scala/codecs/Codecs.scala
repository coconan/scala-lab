...
trait DecoderInstances:
  ...
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    ...
    def decodeAndPrepend(item: Json, previouslyDecodedItems: List[A]): Option[List[A]] =
      item.decodeAs match
        case Some(data) => Some(data :: previouslyDecodedItems)
        case None => None
...
trait ContactsCodecs:
  ...
  given Encoder[Contacts] =
    Encoder.fromFunction {
      case Contacts(people) => ObjectEncoder.field("people")(using Encoder.listEncoder[Person]).encode(people)
    }

  // ... then implement the decoder
  given Decoder[Contacts] =
  Decoder.fromFunction {
    case Json.Obj(fields) => Decoder.field("people")(using Decoder.listDecoder[Person]).decode(Json.Obj(fields)) match
      case Some(items) => Some(Contacts(items))
      case _ => None
    case _ => None
  }
