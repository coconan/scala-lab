...
final class Wikigraph(client: Wikipedia):
  ...
  def namedLinks(of: ArticleId): WikiResult[Set[String]] =
    client.linksFrom(of).flatMap {
      as => WikiResult.traverse(as.toSeq) {
        a => client.nameOfArticle(a)
      }.map(s => s.toSet)
    }
  ...
  def breadthFirstSearch(start: ArticleId, target: ArticleId, maxDepth: Int): WikiResult[Option[Int]] =
    ...
    def iter(visited: Set[ArticleId], q: Queue[(Int, ArticleId)]): WikiResult[Option[Int]] =
      if q.isEmpty then
        ...
        val next = q.dequeue
        if next._1._1 > maxDepth then
          WikiResult.successful(None)
        else
          if target == next._1._2 then
            WikiResult.successful(Some(next._1._1-1))
          else
            client.linksFrom(next._1._2).flatMap {
              as => iter(visited.incl(next._1._2), next._2.enqueueAll(as.diff(visited.incl(next._1._2)).map(id => (next._1._1 + 1, id))))
            }.fallbackTo(iter(visited.incl(next._1._2), next._2))
    end iter
  ...
  def distanceMatrix(titles: List[String], maxDepth: Int = 50): WikiResult[Seq[(String, String, Option[Int])]] =
    WikiResult.traverse(titles.flatMap {
      t1 => titles.map(t2 => (t1, t2))
    }.filterNot((t1, t2) => t1.equals(t2))) {
      (t1, t2) => client.searchId(t1).zip(client.searchId(t2))
        .flatMap((a1, a2) => breadthFirstSearch(a1, a2, maxDepth).map( v => (t1, t2, v)))
    }
end Wikigraph
