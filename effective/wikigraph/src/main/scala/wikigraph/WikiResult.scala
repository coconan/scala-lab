...
case class WikiResult[A](value: Future[Either[Seq[WikiError], A]]):
  ...
  def map[B](f: A => B)(using ExecutionContext): WikiResult[B] =
    WikiResult(value.flatMap {
      case Right(v) => Future.successful(Right[Seq[WikiError], B](f(v)))
...
object WikiResult:
  ...
  def traverse[A, B](as: Seq[A])(f: A => WikiResult[B])(using ExecutionContext): WikiResult[Seq[B]] =
    as.map(f).foldLeft[WikiResult[Seq[B]]](WikiResult.successful(List.empty)) {
      (acc, result) => acc.zip(result).map {
        (bs, b) => bs :+ b
      }
    }
