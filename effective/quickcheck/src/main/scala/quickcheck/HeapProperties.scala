...
trait HeapProperties(val heapInterface: HeapInterface):
  ...
  val meldingSmallHeaps: (String, Prop) =
    ...
    forAll { (x: Int, y: Int) =>
      ...
      val meldedHeap: List[Node] = meld(insert(Math.min(x, y), insert(Math.min(x, y), empty)),
        insert(Math.max(x, y), insert(Math.max(x, y), empty)))
  ...
  val meldingHeaps: (String, Prop) =
    def check(heap1: List[Node], heap2: List[Node], meldedHeap: List[Node]): Boolean =
      ...
      else if !isEmpty(heap1) && findMin(heap1) == findMin(meldedHeap) then
        check(deleteMin(heap1), heap2, deleteMin(meldedHeap))
      else if !isEmpty(heap2) && findMin(heap2) == findMin(meldedHeap) then
