...
trait Solver extends GameDef:
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors.filter((b, _) => !explored.contains(b))
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if initial.isEmpty then LazyList.empty
      ...
      val frontier =
        for
          next <- newNeighborsOnly(neighborsWithHistory(initial.head._1, initial.head._2), explored)
        yield next
      initial.head #:: from(initial.tail ++ frontier, explored.incl(initial.head._1))
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.filter((b, _) => done(b))
