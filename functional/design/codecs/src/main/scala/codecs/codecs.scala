...
trait DecoderInstances:
  ...
  given Decoder[Int] =
    Decoder.fromFunction {
      case Json.Num(n) => if n.isValidInt then Option(n.toInt) else Option.empty
      case _ => Option.empty
    }
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction {
      case Json.Arr(arr) => Option(arr.map(e => decoder.decode(e).get))
      case _ => Option.empty
    }
...
trait ContactsCodecs:
  ...
  given Decoder[Contacts] =
    Decoder.field[List[Person]]("people")
      .transform[Contacts](ps => Contacts(ps))
