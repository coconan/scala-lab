...
def boxBlurKernel(src: Img, x: Int, y: Int, radius: Int): RGBA =
  ...
  do {
      n = n + 1
      sumRed = sumRed + red(src(i, j))
      sumGreen = sumGreen + green(src(i, j))
      sumBlue = sumBlue + blue(src(i, j))
      sumAlpha = sumAlpha + alpha(src(i, j))
  }
  rgba(sumRed/n, sumGreen/n, sumBlue/n, sumAlpha/n)
