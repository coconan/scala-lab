...
class Empty extends TweetSet:
  ...
  def mostRetweeted: Tweet = throw java.util.NoSuchElementException("Empty TweetSet")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweetedAcc(acc: Tweet): Tweet =
    right.mostRetweetedAcc(left.mostRetweetedAcc(if acc.retweets > elem.retweets then acc else elem))
